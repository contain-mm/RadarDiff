...
from models.modules import TCD_Net,CasualContextNet
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        # calculations for posterior q(x_{t-1} | x_t, x_0),它表示在前向扩散过程中，已知当前噪声状态 xt和原始数据 x0时，前一时刻状态 xt-1的条件概率分布
        ...
        # auto-normalization of sh_data [0, 1] -> [-1, 1] - can turn off by setting it to be False
    ...
    def sample(self, frames_in, T_out, return_all_timesteps=False):
        ...
        pre_frag = frames_in
        state_shape = (B, c, h, w)
        self.ctx_net.init_state(state_shape, device)
        seg_len=T_in
        for frag_idx in tqdm(range(T_out // seg_len), desc="sampling frags:", disable=(frames_in.device == 0)):
            cond = pre_frag
            ctx = self.ctx_net.scan_ctx(pre_frag)
            y = sample_fn(
                    cond=cond,
                    ctx=ctx,
    ...
    def forward(self, frames_in, frames_gt,t=None):
        ...
        diff_loss = 0.
        t = t if t is not None else torch.randint(0, self.num_timesteps, (B,), device=device).long()
        state_shape = (B, c, h, w)
        self.ctx_net.init_state(state_shape, device)
        seg_len = T_in
        for frag_idx in range(T_out // seg_len):
            s_label = frames_gt[:, frag_idx * seg_len: (frag_idx + 1) * seg_len]  
            cond = pre_frag
            ctx = self.ctx_net.scan_ctx(pre_frag)
            _, noise_loss = self.p_losses(s_label, t, cond=cond, ctx=ctx ,idx=torch.full((B,), frag_idx, device=device, dtype=torch.long))
            diff_loss += noise_loss
            pre_frag = frames_gt[:, frag_idx * seg_len: (frag_idx + 1) * seg_len]
        diff_loss /= (T_out // seg_len)
        loss= diff_loss
        return loss
    ...
    def p_losses(self, x_start, t, noise=None, offset_noise_strength=None, ctx=None, idx=None, cond=None):
        ...
        model_out = self.model(x, t, cond=cond, idx=idx, ctx=ctx)
...
def get_model(config):
    ...
    diffusion = GaussianDiffusion(
        ...
        sampling_timesteps=config["time_steps"],
    )
    return diffusion
